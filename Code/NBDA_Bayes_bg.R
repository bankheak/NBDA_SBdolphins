...
# Save data
write.csv(group_data, "group_data_bg.csv")
write.csv(ILV_all, "ILV_BG_subset.csv")
...
gbi_bg <- create_gbi(group_data_list) # Run the function
saveRDS(gbi_bg, "gbi_bg.RData") # Save data
...
nxn_bg <- create_nxn(gbi_bg) # Run the function
saveRDS(nxn_bg, "nxn_bg.RData") # Save data

#### PART 2: Calculate acquisition and predictor data ####

# Read in all network data
nxn <- readRDS("nxn_bg.RData")
...
# Read in group data
group_data <- read.csv("group_data_bg.csv")
...
ILV_all <- read.csv("ILV_BG_subset.csv")
...
write.csv(event_data, "event_data_bg.csv") # Save data
...
ILV_c <- data.frame(id = ILV_all$Alias,
                    sex = ILV_all$Sex)
write.csv(ILV_c, "ILV_c_bg.csv") # Save data
...
write.csv(ILV_tv, "ILV_tv_bg.csv") # Save data
...
write.csv(HI_matrix, "HI_matrix_bg.csv") # Save data
...
saveRDS(edge_list, "edge_list_bg.RData") # Save data


#### PART 3: Aggregate data for model input ####

# Read in event data
event_data <- read.csv("event_data_bg.csv")

# Read in edge list
edge_list <- readRDS("edge_list_bg.RData")

# Read in ILV data
ILV_tv <- read.csv("ILV_tv_bg.csv")
ILV_c <- read.csv("ILV_c_bg.csv")

# Read in weighted data
HI_matrix <- read.csv("HI_matrix_bg.csv")
...
saveRDS(data_list, "data_list_bg.RData") # Save data
...
STb_save(fit, output_dir = "cmdstan_saves_bg", name="my_first_fit") # Save model


#### PART 5: Summarize Outputs ####

# Read in model output
fit <- readRDS('cmdstan_saves_bg/my_first_fit.rds')
