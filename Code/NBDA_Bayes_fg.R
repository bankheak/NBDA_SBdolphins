...
STb_save(full_fit, output_dir = "cmdstan_saves_fg", name="my_first_fit") # Save model

# Fit the asocial model
asocial_fit <- fit_STb(data_list,
                       "AS_model_strong_priors_fg.stan",
                       parallel_chains = 4,
                       chains = 4,
                       cores = 4,
                       iter_warmup = 2000,
...
STb_save(asocial_fit, output_dir = "cmdstan_saves_fg_as", name="my_first_fit") # Save model

# Model comparisons
## Read in data from HPC
full_fit <- readRDS('cmdstan_saves_fg/my_first_fit.rds')
asocial_fit <- readRDS('cmdstan_saves_fg_as/my_first_fit.rds')
...
# Save data
saveRDS(plot_df, "plot_df_fg.RData")

# 5. Plot
pal <- c("Individual Learning" = "#A67C00", "Social Learning" = "#4A90D9")
